...
leaves alone. In class we found fixed points by iterating: start somewhere, apply `f` over and
over, and see where you land. That works, but only sometimes, and it doesn't tell you *where*
the fixed point is if the iteration runs away.

In this assignment you'll find fixed points a different way — with **binary search** — and then
write a second program to determine whether each fixed point is **attracting** (nearby points
...
and you are hunting for a zero of `g`. If `g(lo)` is positive and `g(hi)` is negative, then
somewhere between `lo` and `hi` the value of `g` crosses zero — and binary search will pin it
down. This is the same halving logic you used to search a sorted array, applied to a
continuous interval instead of an index range.

Write everything in a single file, `FixedPoint.java`.
...
Binary searches the interval `[lo, hi]` for a fixed point of `f` and returns it. Each pass,
compute the midpoint `guess`. Then you compare `guess` to `f(guess)` and update either `lo` or
`hi`. (Note you don't add `+1` or `-1` here because we're not searching an array).
...
You'll also want a constant near the top of the class. You can adjust its value:
...
Things to notice and be ready to answer:

1. Roughly how many steps does it take? Does that number depend much on which function you're
   searching, or mostly on something else?
2. What happens if you hand it an interval where `f(lo) - lo` and `f(hi) - hi` are *both*
   positive? Try it. Explain what the code does and why.
3. Why does the loop test `f(guess) - guess` rather than testing whether `lo` and `hi` have
   gotten close together? Would the other test also work?
...
Turn in a table with three columns: the function, the fixed point to nine decimal places, and
attracting or repelling.

Then:

6. Two of these fixed points are numbers you already know by another name. Which two, and what
   are they? (Compare your decimals against something familiar.)

7. Function 4 converges under iteration *much* faster than the others. Iterate it by hand from
   `x = 1` a few times and see. What's special about it?
8. Sort your seven functions into the attracting group and the repelling group. Sketch each
   ...
   can — this is the interesting part of the assignment.
...
9. With `f(x) = 3.2x(1 - x)`, find both points of the 2-cycle. Verify they map to each other.
10. Change to `f(x) = 3.83x(1 - x)` and find a 3-cycle. There is more than one. Find them all.
11. Some intervals that clearly contain a cycle point cause your search to fail or to return
...
12. Extend `testAttracting` to classify a whole cycle. What does attracting mean for a cycle?
13. Every fixed point of `f` is also a fixed point of `f(f(x))`. So how does a program that
    searches `f(f(x))` know it has found a genuine 2-cycle and not just an old fixed point
    wearing a disguise? Handle this.
14. Find your own function with an interesting cycle: a 4-cycle, a cycle that some points reach
